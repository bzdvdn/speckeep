...
- **Express-полоса**: крошечные/низкорисковые правки пропускают `plan.md` (и `data-model.md`). `/spk-tasks` выводит задачи напрямую из `spec.md`; плановые решения уходят в `tasks.md` → `## Implementation Context`. Lifecycle позволяет фиче с `spec.md + tasks.md` дойти до `implement` и `archive` без отдельного плана.
