...
`inspect` перед `plan` опционален. Планирование может продолжаться без inspect report, если спецификация ясная и низкорисковая.
Если inspect report присутствует, для продолжения он должен иметь валидный неблокирующий статус (`pass` или `concerns`).
...
Предпочтительный формат: `@sk-task <slug>#<TASK_ID>: <Описание> (<AC_ID>)`
- Для тестов: `@sk-test <slug>#<TASK_ID>: <НазваниеТеста> (<AC_ID>)`
- Если одну задачу подтверждают несколько тестов/кейсов, `@sk-test` должен быть на каждом таком тесте/кейсе.
- Стиль размещения зависит от языка: Go/Java/C#/C/C++ обычно используют комментарий над declaration; Python — `# @sk-*` первой строкой внутри `def/class/test`; JS/TS для `test()/it()` — первой строкой внутри callback.
