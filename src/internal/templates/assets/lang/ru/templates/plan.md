...
- `data-model.md` опционален on demand: создавайте его только когда фича реально меняет модель/состояние/персистентность; иначе фиксируйте `Data model: no change` прямо здесь
- Сам `plan.md` опционален для крошечных/низкорисковых правок — закрывайте такие фичи через `spec.md` + `tasks.md` (express-режим)
