...
- Placement аннотаций:
  ...
  - никогда не ставьте `@sk-task`/`@sk-test` над `package`, `import` или file-header comment только ради того, чтобы «trace был в файле».
  - соблюдайте idiomatic language style: Go/Java/C#/C/C++ обычно требуют комментарий непосредственно над declaration; Python — `# @sk-task`/`# @sk-test` первой строкой внутри тела `def/class/test`; `test(...)`/`it(...)` в JS/TS — первой строкой внутри callback.
- Traceability обязательно для implement:
  ...
  - если одну задачу подтверждают несколько тестов/кейсов, `@sk-test` должен быть на каждом таком тесте/кейсе.
