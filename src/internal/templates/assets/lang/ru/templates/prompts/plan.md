...
Outputs: `<specs_dir>/<slug>/plan.md`, `<specs_dir>/<slug>/plan.digest.md`, и при необходимости `<specs_dir>/<slug>/data-model.md`, `<specs_dir>/<slug>/contracts/*`, `<specs_dir>/<slug>/research.md`.
...
- **Сначала запишите `<specs_dir>/<slug>/plan.digest.md`** (обязательно, всегда, даже при patch). Одна строка на каждый `DEC-*`, затем блок `Surfaces:` (название surface + ключевые файлы, одна строка каждый). Если `DEC-*` нет — пишите только `Surfaces:`. Пример:
  ```
