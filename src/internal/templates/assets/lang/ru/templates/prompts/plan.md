...
Inputs: `project.constitution_file` (по умолчанию `CONSTITUTION.md`, или `.speckeep/constitution.summary.md` если есть), `<specs_dir>/<slug>/spec.md`, `<specs_dir>/<slug>/inspect.md` (должен быть `pass`).
...
- Запишите `<specs_dir>/<slug>/plan/plan.digest.md`: одна строка на каждый `DEC-*` (формат `DEC-NNN: <краткое описание ≤10 слов>`), затем список `Surfaces:` (название surface + основные файлы, одна строка каждый). Никакой детализации.
