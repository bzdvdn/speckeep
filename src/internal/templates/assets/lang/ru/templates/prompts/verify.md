...
- Формат evidence: перечисляйте строки `T* -> proof` (и `AC-* -> T*`, когда релевантно). Proof должен быть наблюдаемым: путь к файлу, имя/вывод теста, вывод команды, или артефакт в `<specs_dir>/<slug>/...`.
