...
func CheckTasksReady(ctx context.Context, cfg config.Config, root, slug string) (CheckResult, error) {
	...
	checkOptionalFile(&result, planDisplay, planAbs, "no plan.md — express mode: derive tasks from spec.md only (plan is optional for small changes)")
	checkOptionalFile(&result, dataModelDisplay, dataModelAbs, "no data-model.md — optional on-demand: create it only when the feature changes data model/state, otherwise keep the note in plan.md")
