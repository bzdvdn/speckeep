...
- `speckeep list-archive` выводит архивированные фичи из `.speckeep/archive/`; показывает одну запись на slug (последний снимок) со статусом, датой и причиной; поддерживает `--status` для фильтрации по статусу, `--since <YYYY-MM-DD>` для фильтрации по дате и `--json` для автоматизации.
...
`.speckeep/specs/eksport-otchetov-v-csv/spec.md`:
...
- `.speckeep/specs/eksport-otchetov-v-csv/inspect.md` — вердикт `pass`, все AC в формате G/W/T
- `.speckeep/specs/eksport-otchetov-v-csv/summary.md` — компактная таблица AC, которую используют implement и verify вместо полного spec
...
`.speckeep/specs/eksport-otchetov-v-csv/plan/tasks.md`:
